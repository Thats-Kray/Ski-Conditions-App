...
| 4 | **Comments are reportable** via the existing `reportContent("activity_comment", commentId, reason)` path. | Kyle's choice. `moderation_flags.content_type` has no CHECK constraint restricting values (confirmed against `migrations/026_verification_infrastructure.sql`), so this needs no schema change — just wiring, matching the precedent already live on Board posts. |
